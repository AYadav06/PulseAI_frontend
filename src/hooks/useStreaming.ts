...
interface UseStreamingReturn {
  ...
  error: string | null;
  sendMessage: (prompt: string, model?: string, conversationId?: string) => Promise<string | undefined>;
  stopGeneration: () => void;
}
...
export const useStreaming = (): UseStreamingReturn => {
  ...
  const sendMessage = useCallback(async (
    ...
    const assistantMessagePlaceholder: ChatMessage = { role: "assistant", content: "", streaming: true };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.message || `Server returned error: ${response.statusText}`);
      }
      ...
      if (err.name === "AbortError") {
        ...
        setMessages((prev) => {
          ...
          if (lastIdx >= 0 && updated[lastIdx].role === "assistant") {
            ...
            updated[lastIdx] = { ...updated[lastIdx], streaming: false, error: true };
          }
          return updated;
        });
      }
